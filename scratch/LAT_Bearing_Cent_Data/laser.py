...
def load_data(fname: str):
    ...
    data = np.genfromtxt(
        fname, dtype=dtype, usecols=(1, 2, 3, 4, 5, 6, 9), delimiter=",", skip_header=1
    )
...
for name, dat in bearing.items():
    ...
    plt.scatter(
        dat["theta"][direction == 1],
        dat["deviation"][direction == 1],
        label=f"{name}_forwards",
    )
    plt.scatter(
        dat["theta"][direction == -1],
        dat["deviation"][direction == -1],
        label=f"{name}_backwards",
    )
plt.legend()
plt.show()
