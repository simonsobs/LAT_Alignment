...
from megham.transform import get_rigid, get_affine, decompose_rotation, decompose_affine, apply_transform
...
hwfe_factors = {"secondary": [0.00034, 0.00076, 0.0029, 0.044, 0.042, 0.014], "receiver": [0.0, 0.0, 0.0026, 0.0, 0.0, 0.0]}
mm_to_um = 1000
...
def get_hwfe(data, get_transform, add_err=False) -> float:
    ...
    for element in elements:
        ...
        data_m1[f"{element}_ref"] = coord_transform(data_m1[f"{element}_ref"], "opt_global", "opt_primary")

    # Transform for M1 perfect
    aff_m1, sft_m1 = get_transform(data_m1["primary"][data_m1["primary_msk"]], data_m1["primary_ref"][data_m1["primary_msk"]], method="mean")
...
def main():
    ...
    parser.add_argument("--affine", "-a", action="store_true", help="Pass to compute affine instead of rigid rotation")
    parser.add_argument("--n_draws", "-n", default=10000, type=int, help="Number of draws from the error distribution to do when estimating HWFE error")
    parser.add_argument(
    ...
    plt.hist(hwfe_werr, density=True, bins='auto', label=f"Mean: {np.mean(hwfe_werr):.2f}\nSTD: {np.std(hwfe_werr):.2f}", alpha=.7)
