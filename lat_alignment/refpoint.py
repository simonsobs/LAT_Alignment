    ...
    def reorder(self, names: NDArray[np.str_], pad: bool=False) -> Self:
        ...
        if np.sum(inself) != len(names):
            if not pad:
                ...
                tods += [RefTOD(n, null_dat.copy(), self.angle.copy()) for n in names[~inself]]
...
class RefCollection:
    ...
    @classmethod
    def construct(cls, data : dict[str, list[RefTOD]], logger : logging.Logger, pad: bool=False) -> Self:
        """
