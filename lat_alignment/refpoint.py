...
class RefElem:
    ...
    def data(self):
        data = np.swapaxes(
            np.atleast_3d(np.array([e.data for e in self.tods])), 0, 1,
        )
        return data 
    ...
    def reorder(self, names, pad=False):
        if len(self.tods) == 0:
        ...
        if np.sum(inself) != len(names):
            if not pad:
                ...
                null_ang = np.zeros_like(tods[0].angle, np.float64) + np.nan
                tods += [RefTOD(n, null_dat.copy(), null_ang.copy()) for n in names[~inself]]
    ...
    def _check(self):
        for elem in self.values():
            ...
            if not (np.isclose(elem.angle, self.angle) | np.isnan(elem.angle)).all():
                raise ValueError("Angles don't agree!")
            if not (np.isclose(elem.direction, self.direction) | np.isnan(elem.direction)).all():
                raise ValueError("Directions don't agree!")

    @classmethod
    def construct(cls, data : dict[str, list[RefTOD]], logger, pad=False):
        npoints = np.hstack( [ [point.npoints for point in data[elem]] for elem in data.keys() ])
        if not np.all(npoints == npoints[0]):
            ...
            master_angle = [ [ point.angle for point in data[elem] ] for elem in data.keys() ]
